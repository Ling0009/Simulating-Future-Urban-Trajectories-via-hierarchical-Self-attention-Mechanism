...
def train_epoch(model, optimizer, encoder_input, dev, PAD_IDX):
    model.train()

    src = encoder_input.to(dev)
    tgt = encoder_input.to(dev)

    tgt_input = torch.cat((torch.zeros(tgt[0].shape).unsqueeze(0).to(dev), tgt[:-1, :]), dim=0)
...
def evaluate(model, encoder_input, dev, PAD_IDX):
    model.eval()
...
def train(transformer, optimizer, encoder_inputs, PAD_IDX, dev):
    encoder_inputs = torch.transpose(encoder_inputs, 0, 1)

    train_loss, embedding = train_epoch(transformer, optimizer, encoder_inputs, dev, PAD_IDX)
    val_loss = evaluate(transformer, encoder_inputs, dev, PAD_IDX)
...
def build_model(frame_dim, cell_num, dev):
    ...
    transformer = tf.Seq2SeqTransformer(NUM_ENCODER_LAYERS, NUM_DECODER_LAYERS, NUM_EMB_SIZE, EMB_SIZE,
                                        NHEAD, SRC_VOCAB_SIZE, TGT_VOCAB_SIZE, FFN_HID_DIM, use_embed=True)
